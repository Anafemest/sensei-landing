...
// Driven by the flow STRIP's viewport position (not the whole section),
// so step 04 ignites while the strip is still on screen — independent of
// how much trust content (standards + FAQ) lives below.
const THRESHOLDS = [0.42, 0.5, 0.58, 0.66];

// "Closed-perimeter" guarantees — surfaced right under the flow so the
// security story sits next to the workflow that depends on it.
const PERIMETER = [
{ label: "Air-gapped",
...
function Flow() {
  ...
  React.useEffect(() => {
    const onScroll = () => {
      ...
      // Drive progress off the flow STRIP, not the whole section.
      // 0 → strip just entering viewport bottom
      // 1 → strip just left viewport top
  ...
  return (
    <section className="section" id="flow" ref={sectionRef}>
      <div className="container">
        ...
        {/* Closed-perimeter guarantees — three pillars that frame the
              flow above as something that runs entirely inside your contour. */}
        ...
        <div className="sec-faq" id="security">
          <div className="sec-faq__head">
            ...
            <h3>Часто Задаваемые Вопросы <em> FAQ</em></h3>
